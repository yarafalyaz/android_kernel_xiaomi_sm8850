...
impl GenlMsg {
    ...
    pub fn put_string(&mut self, attrtype: c_int, value: &CStr) -> Result {
        self.put(attrtype, value.as_bytes_with_nul())
    }
...
impl Family {
    ...
    pub const fn const_new(
        ...
        let mut genl_family = bindings::genl_family {
            ...
            n_mcgrps,
            // SAFETY: All zeroes is safe for the `genl_family` type.
...
impl MulticastGroup {
    ...
    pub const fn const_new(name: &CStr) -> MulticastGroup {
        // SAFETY: All zeroes is safe for the `genl_multicast_group` type.
        let mut group: bindings::genl_multicast_group =
            unsafe { core::mem::MaybeUninit::zeroed().assume_init() };
